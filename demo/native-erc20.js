const {
  ...
  const { parseUnits, formatEther, parseEther} = require('../lib.commonjs/utils')
...
const Web3 = require('web3')
  ...
  const getJoyTokenBalance = async (axonAddress) => {
    const ERC20ABI = require('./erc20_abi.json');
    const web3 = new Web3(new Web3.providers.HttpProvider(AXON_RPC_RUL))
    const contract = new web3.eth.Contract(ERC20ABI, JOY_ERC20_CONTRACT_ADDRESS);
    return await contract.methods.balanceOf(axonAddress).call();
  }

  const transferJoyTokenWithMainkey = async () => {
    ...
    let joyTokenBalance = await getJoyTokenBalance(axonAddress)
    console.log("Before transferring, JOY Token balance: ", formatEther(joyTokenBalance))
  
    let axonTx = new Transaction()
    axonTx.from = axonAddress
    axonTx.to = JOY_ERC20_CONTRACT_ADDRESS
    axonTx.value = 0
    axonTx.chainId = 2022
    axonTx.type = 0
    axonTx.from = axonAddress
    ...
    const rawData = iface.encodeFunctionData('transfer', [TO_AXON_ADDRESS, parseEther("2")])
    axonTx.data = rawData

    axonTx = await signAxonTxWithMainkey(lock, axonTx)
  
    const ret = await provider.send('eth_sendRawTransaction', [axonTx.serialized])
    console.log("Transfer JOY erc20 token with mainkey unlock: ", JSON.stringify(ret))

    setTimeout(async () => {
      joyTokenBalance = await getJoyTokenBalance(axonAddress)
      console.log("After transferring, JOY Token balance: ", formatEther(joyTokenBalance))
    }, 10000)
    
  }

  transferJoyTokenWithMainkey()
