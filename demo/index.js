const { addressFromPrivateKey, blake256, Collector, getJoyIDCellDep, keccak160, keyFromPrivate, SigAlg, signSecp256r1Tx, utf8ToHex, remove0x, calcSignedWitnessLock } = require('@nervina-labs/joyid-sdk');
...
const start = async () => {
    ...
    const sigR = [
        [
            toBuffer(signedTx.cellDeps[0].outPoint.txHash),
            toBuffer(signedTx.cellDeps[0].outPoint.index),
            1,
        ],
        [],
    ...
    const sigS = [
        [
            toBuffer(signedTx.witnesses[0].lock),
            [],
            [],
        ]
    ]
